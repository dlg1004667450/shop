...
function add_order(){
    ...
    if (check_sku_num()===true) {
        ...
        window.location.href='/web/cart/confirm?sku_id='+sku_id+'&buy_num='+buy_num+'&buy_type=sku';
    }
}
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.status=='y') {
                    ...
                    if (data.info=='请登录后操作') {
                        layer.msg('正在跳转登录');
                        window.parent.parent.location.href='/web/welcome/login?redirect_url='+window.location.href;
                    } else {
                        setTimeout(function(){
                            //隐藏选择框
                            $('.masker').removeClass('maskershow');
...
function favorite_add(id){
    if (id=='') {
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.status=='y') {
                    $('#godos_favorite_'+id).attr('src','/views/default/skin/images/is_favorite.png');
                    layer.msg('收藏成功');
...
function favorite_delete(id,goods_id){
    if (id=='') {
        ...
        $.ajax({
            ...
            success: function(data){
                if (data.status=='y') {
                    $('#godos_favorite_'+goods_id).attr('src','/views/default/skin/images/no_favorite.png')
                    layer.msg('收藏取消');
                } else {
                    if (data.info=='请登录后操作') {
                        layer.msg('正在跳转登录');
