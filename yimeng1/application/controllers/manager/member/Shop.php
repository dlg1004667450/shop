...
class Shop extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($shop_name)) $where_data['where']['shop_name'] = $shop_name;
        $search_where = array(
            'status' => $status,
            'username' => $username,
            'shop_name'=>$shop_name
        ...
        display('manager/member/shop/list.html');
    }
    ...
    public function add($m_id)
    {
        ...
        if (!empty($m_id)) {
            ...
            display('manager/member/shop/add.html');
        } else {
            error_json('请先注册会员');
        }
    }
    ...
    public function view($m_id)
    {
        ...
        assign('item', $member_shop);
        $this->load->helpers('upload_helper');//加载上传文件插件
        display('manager/member/shop/view.html');
    }
}
