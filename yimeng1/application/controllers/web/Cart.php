...
class Cart extends CI_Controller
{
    ...
    {
        display('default/cart/index.html');
    }
    ...
    public function confirm()
    {
        ...
        if (!empty($sku_id) && !empty($buy_type)) {
            ...
            $address_url = site_url('/web/member/address?redirect_url=' . urlencode(get_now_url()));
            ...
            if (is_array($shop_list)) {
                ...
                display('default/cart/confirm.html');
            } else {
                msg($shop_list);
            }
        } else {
            msg('请选择商品', site_url('/web/cart'));
        }
    }
    ...
    public function commit()
    {
        ...
        if ($all_order_price <= 0) {
            header('location:' . site_url('/web/member/order'));
        } else {
            //直接去支付
            header('location:' . site_url('/api/pay/do_pay?client=web&order_no=' . join(',', $order_no)));
        }
    }
