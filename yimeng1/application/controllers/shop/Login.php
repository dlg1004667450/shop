...
class Login extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!empty($shop_id)) {
            header('location:' . site_url('/shop/welcome'));
        } else {
            $redirect_url = trim($this->input->get('redirect_url', true));//返回链接
            if (empty($redirect_url)) $redirect_url = site_url('/shop/welcome');
            assign('redirect_url', $redirect_url);
            display('shop/login.html');
        }
    }
    ...
    public function login_act()
    {
        ...
        if (!empty($username) && !empty($password)) {
            ...
            if ($shop_data['username'] == '') {
                ...
                $this->session->set_userdata('shop_id', array('shop_id' => $shop_data['id'], 'type' => $shop_data['type']));
    ...
    {
        $shop_id = $this->session->userdata('shop_id');
        shop_admin_log_insert($shop_id . '退出系统');
        $this->session->unset_userdata('shop_id');
        echo "<script>alert('您已经退出登陆。');parent.window.location.href='" . site_url('/shop/login') . "'</script>";
    }
