...
class Order_model extends CI_Model
{
    ...
    public function update_store_nums($sku_id, $goods_num, $type = 'add')
    {
        ...
        if (!empty($sku_id) && !empty($goods_num)) {
            ...
            //更新sku库存
            $this->loop_model->update_id('goods_sku', array('set' => array('store_nums', 'store_nums' . $type . $goods_num)), $sku_id);
            //更新商品库存
            $this->loop_model->update_id('goods', array('set' => array('store_nums', 'store_nums' . $type . $goods_num)), $sku_data['goods_id']);
        }
    }
    ...
    public function update_goods_sale($goods_id, $goods_num, $type = 'add')
    {
        ...
        if (!empty($goods_id) && !empty($goods_num)) {
            ...
            //更新商品销量
            $this->loop_model->update_id('goods', array('set' => array('sale', 'sale' . $type . $goods_num)), $goods_id);
        }
    }
