...
class User_model extends CI_Model
{
    ...
    public function update($data_post = array())
    {
        ...
        if (!empty($data_post['id'])) {
            ...
            $this->im_api->update_user($data_post['id'], $password_data['headimgurl'], $update_data['full_name']);
        } else {
            //增加数据
            if (empty($data_post['username']) || empty($data_post['password'])) {
                ...
                if (!empty($member_data)) {
                    ...
                    //注册im账号
                    $this->load->library('im_api');
                    $update_data['full_name'] != '' ? $full_name = $update_data['full_name'] : $full_name = $insert_data['username'];
                    $this->im_api->add_user($m_id, $insert_data['headimgurl'], $update_data['full_name']);
                }
            }
        }
        if (!empty($res)) {
